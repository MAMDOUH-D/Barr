...
const fbGet = async (key) => {
  try {
    const snap = await getDoc(doc(db, "barr", key));
...
const fbSet = async (key, value) => {
  try {
    await setDoc(doc(db, "barr", key), { value });
  } catch (e) { console.log("Firebase set error:", e); }
};
...
export default function FatherCare() {
  ...
  const loadData = async () => {
    try {
      const [sched, logs_, notes_, pins_] = await Promise.all([
        fbGet("schedule"),
        fbGet(`logs_${today}`),
        fbGet(`notes_${today}`),
        fbGet("pins"),
  ...
  const savePins = async (newPins) => {
    setPins(newPins);
    fbSet("pins", JSON.stringify(newPins));
    showNotif("✅ تم حفظ الرقم السري");
  };

  const saveSchedule = async (newSched) => {
    setSchedule(newSched);
    fbSet("schedule", JSON.stringify(newSched));
  };
  ...
  const markDone = async (item) => {
    ...
    setLogs(newLogs);
    fbSet(`logs_${today}`, JSON.stringify(newLogs));
    setSavingId(null);
  ...
  const addNote = async () => {
    ...
    setNotes(newNotes);
    fbSet(`notes_${today}`, JSON.stringify(newNotes));
    setNoteText("");
